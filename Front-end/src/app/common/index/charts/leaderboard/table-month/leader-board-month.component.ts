...
@Component({
  selector: 'app-leaderboard-month',
  templateUrl: '../leaderboard-table.component.html',
  styleUrls: ['../leaderboard.component.css']
})
export class LeaderBoardOneMonthComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    const dateFromMonth: Date = new Date(Date.now());
    dateFromMonth.setMonth(dateFromMonth.getMonth() - 1);
    this.getDataFromInterval(new Date(dateFromMonth), new Date(Date.now())).subscribe(value => {
